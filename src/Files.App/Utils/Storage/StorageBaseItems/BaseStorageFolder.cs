...
namespace Files.App.Utils.Storage
{
	public abstract class BaseStorageFolder : IBaseStorageFolder
	{
		...
		public static IAsyncOperation<BaseStorageFolder> GetFolderFromPathAsync(string path)
		{
			return
				AsyncInfo.Run(async (cancellationToken)
					=> await AnsiZipStorageFolder.FromPathAsync(path) ?? await ZipStorageFolder.FromPathAsync(path) ?? await FtpStorageFolder.FromPathAsync(path) ?? await ShellStorageFolder.FromPathAsync(path) ?? await SystemStorageFolder.FromPathAsync(path));
		}
